...
module YearHighestTemperatue
  module YearHighestTemperatureHelper
    def year_highest_temp_day_helper(year_weather_files)
      data = {
        date: '2000-01-21',
        temperature: 0
      }
    ...
    def year_highest_temp_getter(lines, data, max_temp_index)
      lines.each do |line|
    ...
    def year_most_humid_day_helper(year_weather_files)
      data = {
        date: '2000-01-21',
        temperature: 0
      }
    ...
    def year_humid_temp_getter(lines, data, max_temp_index)
      lines.each do |line|
        next if line.split(',')[max_temp_index].nil?
    ...
    def year_lowest_temp_day_helper(year_weather_files)
      data = {
        date: '2000-01-21',
        temperature: 0
      }
      year_weather_files.each do |file|
        File.open(file, 'r') do |opened_file|
          ...
          lines = files_data[1, files_data.length - 1]
          data = year_lowest_temp_getter(lines, data, max_temp_index)
        end
      end
      "Lowest : #{data[:temperature]} C on #{Date.parse(data[:date]).strftime('%B %d')}"
    end

    def year_lowest_temp_getter(lines, data, max_temp_index)
      lines.each do |line|
        next if line.split(',')[max_temp_index].nil?
  ...
  class YearHighestTemperatueClass
    extend YearHighestTemperatureHelper
    def year_highest_temp_day(file_name, year)
      files = Dir["#{file_name}/**"]
      year_weather_files = files.map { |file| file if file.include? "#{file_name}_#{year}" }.compact!
      YearHighestTemperatueClass.year_highest_temp_day_helper(year_weather_files)
    end

    def year_lowest_temp_day(file_name, year)
      files = Dir["#{file_name}/**"]
      year_weather_files = files.map { |file| file if file.include? "#{file_name}_#{year}" }.compact!
      YearHighestTemperatueClass.year_lowest_temp_day_helper(year_weather_files)
    end

    def year_most_humid_day(file_name, year)
      files = Dir["#{file_name}/**"]
      year_weather_files = files.map { |file| file if file.include? "#{file_name}_#{year}" }.compact!
      YearHighestTemperatueClass.year_most_humid_day_helper(year_weather_files)
    end
  end
end
